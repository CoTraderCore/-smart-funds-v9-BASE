...
contract ExchangePortalLightV2 is ExchangePortalInterface, Ownable {
  ...
  IWhaleX public whaleX;


  // Enum
  // NOTE: You can add a new type at the end, but DO NOT CHANGE this order,
  // because order has dependency in other contracts like ConvertPortal
  enum ExchangeType { Paraswap, Bancor, OneInch, OneInchETH, UniswapV2, WhaleX }
  ...
  */
  ...
  function _tradeViaWhaleX(
    address sourceToken,
    address destinationToken,
    uint256 sourceAmount
  ) private
    returns(uint256 destinationReceived){
    // whale X can only buy
    if(IERC20(sourceToken) == IERC20(0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE)){
       whaleX.convert.value(sourceAmount)(
         destinationToken,
         1
       );
    }
